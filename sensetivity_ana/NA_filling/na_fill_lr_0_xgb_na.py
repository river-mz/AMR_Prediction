...
for i, prescription in enumerate(prescription_columns):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kf.split(X, y_binary)):
        ...
        train_data = TensorDataset(torch.tensor(X_train_0.values.astype(np.float32)), torch.tensor(y_train.values.astype(np.float32)))
        test_data = TensorDataset(torch.tensor(X_test_0.values.astype(np.float32)), torch.tensor(y_test.values.astype(np.float32)))
        ...
        y_pred = best_lr_model(torch.tensor(X_test_0.values.astype(np.float32)))
