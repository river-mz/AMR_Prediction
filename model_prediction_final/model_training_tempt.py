...
output_fig_dir = '/home/linp0a/AMR_prediction_pipeline/loss_auc_curves_Oct_4/'
if not os.path.exists(output_fig_dir):
    os.makedirs(output_fig_dir)  # 如果目录不存在，自动创建


output_dir = '/home/linp0a/AMR_prediction_pipeline/model_prediction/model_Ours_Oct_4'
...
for i, prescription in enumerate(prescription_columns):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kf.split(X, y_binary)):
        ...
        lr_param_grid = {
            ...
            'optimizer__weight_decay': [0, 0.01, 0.1],  # L2 regularization
            'batch_size': [32, 64, 128]  # 添加batch size
        }
        # lr_param_grid = {
        #     'lr': [ 0.0001],
        #     'max_epochs': [100],
        ...
        train_split=None,  # We handle the train/test split ourselves
        verbose=0,
        iterator_train__batch_size=32,  # 默认batch size
        iterator_valid__batch_size=32   # 默认batch size
        )   

        grid = GridSearchCV(estimator=model, param_grid=lr_param_grid, cv=2, n_jobs=-1)
        grid_result = grid.fit(X_train.values.astype(np.float32), y_train.values.astype(np.float32).reshape(-1, 1))

        y_proba_lr = grid.predict_proba(X_test.values.astype(np.float32))[:, 1]
        best_threshold_lr, best_metrics_lr = optimize_thresholds(y_test.values.astype(np.float32), y_proba_lr, "Logistic Regression", prescription)

        print('grid search result above!')


        # Step 5: Use the best parameters to train a new model with PyTorch
        best_params = grid_result.best_params_
        print("Best Hyperparameters:", best_params)

        # Create a new model with the best hyperparameters
        best_model = LogisticRegressionModule(input_dim=X_train.shape[1])
        optimizer = optim.Adam(best_model.parameters(), lr=best_params['lr'], weight_decay=best_params['optimizer__weight_decay'])
        criterion = nn.BCEWithLogitsLoss()
        ...
        for epoch in range(best_params['max_epochs']):
            ...
            for X_batch, y_batch in train_loader:
                optimizer.zero_grad()
                y_pred_logits = best_model(X_batch)
        ...
        y_pred = best_model(torch.tensor(X_test.values.astype(np.float32)))
        ...
        xgb_param_grid = {'n_estimators': [10, 50, 100, 200], 'max_depth': [3, 5, 10, 20],  'subsample': [0.5, 0.7, 0.8, 1.0]}
