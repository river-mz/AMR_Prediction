...
def predict_with_ci(df, prescription_columns, save_dir = '/home/linp0a/AMR_prediction_pipeline/model_prediction/train_models_for_CI/save_models_for_CI/'):
    print(df.shape)
    ...
    for prescription in prescription_columns:
        ...
        # 加载每个模型并进行预测
        for i in range(11):
            model_path = os.path.join(save_dir, f"xgb_model_{prescription}_run_{i}.pkl")
            print('model ', i)
            print(model_path)
